...
export async function GET(request: Request) {
  ...
  const movies = await prisma.movie.findMany({
    ...
    include: { showtimes: { where: { onSale: true, startsAt: { gte: firstShowtimeToDisplay }, auditorium: { locationId: location.id } }, select: { id: true, startsAt: true, presentation: true, format: true, filmSeries: { select: { id: true, name: true } }, auditorium: { select: { id: true, name: true, capacity: true } }, priceTier: { select: { name: true, ticketPriceMinor: true, feeMinor: true, currency: true } } }, orderBy: { startsAt: "asc" } } },
    orderBy: { title: "asc" },
  });
