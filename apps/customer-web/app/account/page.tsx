        ...
        <div className="account-dashboard">
          ...
          {accountLoading && !account ? (
            ...
            <>
              ...
              <section className="account-memberships" aria-label="My memberships">
                <div className="account-section-heading"><span className="eyebrow">MEMBERSHIP</span><h2>My memberships</h2></div>
                {account?.memberships?.length ? account.memberships.map((membership) => <article className="content-panel account-membership" key={membership.id}><header><div><span className="eyebrow">{membership.organizationName}</span><h3>{membership.plan?.name ?? membership.tier}</h3><p>Member #{membership.membershipNumber}</p></div><span className="status-chip">{statusLabel(membership.status)}</span></header><div className="account-membership__details"><p><strong>{membership.expiresAt ? `Valid through ${new Date(membership.expiresAt).toLocaleDateString()}` : "No expiration"}</strong></p>{membership.plan?.description && <p>{membership.plan.description}</p>}{Array.isArray(membership.plan?.benefits) && <ul>{membership.plan.benefits.map((benefit) => typeof benefit === "string" ? <li key={benefit}>{benefit}</li> : null)}</ul>}</div><div className="account-membership__history"><h4>Enrollment &amp; renewal history</h4>{membership.purchases.map((purchase) => <p key={purchase.id}><span>{purchase.planName} · {new Date(purchase.purchasedAt).toLocaleDateString()} · {purchase.locationName}<small>{money(purchase.taxDeductibleAmountCents, purchase.currency)} potentially tax-deductible</small></span><strong>{money(purchase.amountCents, purchase.currency)}</strong></p>)}{!membership.purchases.length && <p className="secondary-copy">No online membership payments are recorded.</p>}</div><Link className="account-secondary-button" href="/membership">View membership options</Link></article>) : <div className="content-panel account-empty"><p>You do not have a membership yet.</p><Link className="account-secondary-button" href="/membership">Explore memberships</Link></div>}
              </section>
