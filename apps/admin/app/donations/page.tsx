...
export default function DonationsPage() {
  ...
  return <main className="admin-route-page donations-page">
    ...
    <section className="panel donation-history"><div className="dashboard-section-heading"><div><p className="kicker">CONTRIBUTIONS</p><h2>Recent donation activity</h2></div><span>{data.donations.length} shown</span></div><div className="donation-history-table"><header><span>Date</span><span>Donor</span><span>Campaign</span><span>Method</span><span>Amount</span><span>Deductible</span></header>{data.donations.map((donation) => <div key={donation.id}><span>{new Date(donation.receivedAt).toLocaleDateString([], { timeZone: employee.timezone })}</span><span>{donation.customer ? <Link className="donation-donor-link" href={`/customers/${donation.customer.id}`}><strong>{donation.customer.name || donation.donorName || "Registered donor"}</strong><small>{donation.customer.email || donation.donorEmail || "No email"}</small></Link> : <><strong>{donation.donorName || "Guest donor"}</strong><small>{donation.donorEmail || "No email"}</small></>}</span><span>{donation.campaign?.name || "General support"}</span><span>{donation.paymentMethod.toLowerCase()}</span><span>{money(donation.amountCents)}</span><span>{money(donation.taxDeductibleAmountCents)}</span></div>)}</div>{!data.donations.length && <p className="dashboard-empty">No donations have been recorded.</p>}</section>
  </main>;
}
