...
type SearchResults = {
  ...
  customers: Array<{ id: string; name: string | null; email: string | null; phone: string | null; _count: { ticketOrders: number; restaurantTabs: number } }>;
...
export default function GlobalSearchPage() {
  ...
  return <main className="admin-route-page"><section className="panel global-search"><p className="kicker">ADMIN SEARCH</p><h2>Find a customer record</h2><p>Search order numbers, customer names and emails, exact ticket credentials, or gift-card recipient details and last four digits.</p>
    ...
    {results && <div className="global-search-results"><p><strong>{count}</strong> results for “{results.query}”</p>
      ...
      <h3>Customers</h3>{results.customers.map((customer) => <div className="customer-search-record" key={customer.id}><article><div><strong>{customer.name ?? "Guest"}</strong><small>{customer.email ?? "No email"}{customer.phone ? ` · ${customer.phone}` : ""}</small></div><span>{customer._count.ticketOrders} ticket orders · {customer._count.restaurantTabs} dining visits</span><button className="secondary" type="button" onClick={() => void openCustomer(customer.id)} disabled={customerLoadingId === customer.id}>{customerLoadingId === customer.id ? "Loading…" : customerHistory?.id === customer.id ? "Close history" : "View history"}</button></article>{customerHistory?.id === customer.id && <section className="customer-history"><div className="customer-history-summary"><div><strong>{customerHistory.summary.orderCount}</strong><small>Ticket orders</small></div><div><strong>{customerHistory.summary.ticketCount}</strong><small>Tickets</small></div><div><strong>{money(customerHistory.summary.lifetimeSpendCents, customerHistory.summary.currency)}</strong><small>Ticket spend</small></div><div><strong>{customerHistory.summary.diningVisitCount}</strong><small>Dining visits</small></div><div><strong>{money(customerHistory.summary.diningSpendCents, customerHistory.summary.diningCurrency)}</strong><small>Dining spend</small></div><div><strong>{customerHistory.isGuest ? "Guest" : "Account"}</strong><small>Customer type</small></div></div><h4>Ticket history</h4>{customerHistory.historyWindow.ticketOrdersShown < customerHistory.historyWindow.ticketOrdersTotal && <p className="history-window-note">Showing the latest {customerHistory.historyWindow.ticketOrdersShown} of {customerHistory.historyWindow.ticketOrdersTotal} ticket orders.</p>}{customerHistory.ticketOrders.map((order) => <div className="customer-history-order" key={order.id}><div><strong>{order.orderNumber}</strong><small>{new Date(order.createdAt).toLocaleString([], { timeZone: employee.timezone })} · {order.channel} · {order.status}</small></div><div>{order.tickets.map((ticket) => <small key={ticket.id}>{ticket.showtimeSeat.showtime.movie.title} · {ticket.showtimeSeat.seat.label} · {ticket.ticketType.name}</small>)}</div><strong>{money(order.totalCents, order.currency)}</strong><Link href={`/refunds?query=${encodeURIComponent(order.orderNumber)}`}>Open order</Link></div>)}{!customerHistory.ticketOrders.length && <p className="dashboard-empty">No ticket purchases at this location.</p>}<h4>Dining history</h4>{customerHistory.historyWindow.diningVisitsShown < customerHistory.historyWindow.diningVisitsTotal && <p className="history-window-note">Showing the latest {customerHistory.historyWindow.diningVisitsShown} of {customerHistory.historyWindow.diningVisitsTotal} dining visits.</p>}{customerHistory.restaurantTabs.map((tab) => <div className="customer-history-order" key={tab.id}><div><strong>{tab.label ?? tab.showtime?.movie.title ?? "Dining visit"}</strong><small>{new Date(tab.openedAt).toLocaleString([], { timeZone: employee.timezone })} · {tab.fulfillmentMode.replaceAll("_", " ")} · {tab.status}</small></div><div>{tab.orders.flatMap((order) => order.items).map((item, index) => <small key={`${item.menuItem.name}-${index}`}>{item.quantity}× {item.menuItem.name}</small>)}{tab.seats.length > 0 && <small>Seats {tab.seats.map((seat) => seat.showtimeSeat.seat.label).join(", ")}</small>}</div><strong>{tab.totalCents === null ? "Open" : money(tab.totalCents, tab.location.currency)}</strong><span>{tab.showtime?.auditorium.name ?? "Counter"}</span></div>)}{!customerHistory.restaurantTabs.length && <p className="dashboard-empty">No dining visits at this location.</p>}</section>}</div>)}{!results.customers.length && <p className="dashboard-empty">No matching customers.</p>}
