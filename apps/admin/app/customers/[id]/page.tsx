...
type Customer = {
  ...
  membership: { membershipNumber: string; tier: string; status: string; expiresAt: string | null; plan: { id: string; name: string; durationMonths: number; active: boolean } | null; checkouts: Array<{ id: string; planName: string; amountCents: number; taxDeductibleAmountCents: number; currency: string; createdAt: string; receiptSentAt: string | null; location: { id: string; name: string } }> } | null;
  summary: { orderCount: number; ticketCount: number; lifetimeSpendCents: number; currency: string; diningVisitCount: number; diningSpendCents: number; diningCurrency: string; donationCount: number; donationAmountCents: number; donationTaxDeductibleAmountCents: number; donationAverageAmountCents: number; donationFirstReceivedAt: string | null; donationLastReceivedAt: string | null; donationCurrency: string; membershipPurchaseCount: number; membershipSpendCents: number; membershipTaxDeductibleAmountCents: number; membershipCurrency: string };
  ...
  return <main className="admin-route-page customer-profile-page">
    ...
    {customer && <>
      ...
      <section className="series-performance-metrics customer-profile-metrics">
        ...
        <div><span>Giving</span><strong>{money(customer.summary.donationAmountCents, customer.summary.donationCurrency)}</strong><small>{customer.summary.donationCount > 1 ? "Repeat donor" : customer.summary.donationCount === 1 ? "One-time donor" : "No settled gifts"} · {money(customer.summary.donationTaxDeductibleAmountCents, customer.summary.donationCurrency)} deductible</small></div>
        <div><span>Giving history</span><strong>{customer.summary.donationCount ? money(customer.summary.donationAverageAmountCents, customer.summary.donationCurrency) : "—"}</strong><small>{customer.summary.donationCount ? `${customer.summary.donationCount} gifts · ${date(customer.summary.donationFirstReceivedAt!)} to ${date(customer.summary.donationLastReceivedAt!)}` : "No giving history"}</small></div>
      </section>
      <section className="panel customer-profile-history"><div className="dashboard-section-heading"><div><p className="kicker">MEMBERSHIP</p><h2>Enrollment &amp; renewal history</h2></div><span>{customer.summary.membershipPurchaseCount} paid · {money(customer.summary.membershipSpendCents, customer.summary.membershipCurrency)} · {money(customer.summary.membershipTaxDeductibleAmountCents, customer.summary.membershipCurrency)} potentially deductible</span></div>
        {customer.membership?.checkouts.map((purchase) => <article className="customer-history-order" key={purchase.id}><div><strong>{purchase.planName}</strong><small>{dateTime(purchase.createdAt)} · Paid online</small></div><div><small>{purchase.location.name}</small><small>{purchase.receiptSentAt ? `Receipt sent ${dateTime(purchase.receiptSentAt)}` : "Receipt not recorded"}</small></div><strong>{money(purchase.amountCents, purchase.currency)}</strong><span>{money(purchase.taxDeductibleAmountCents, purchase.currency)} potentially tax-deductible</span></article>)}
        {!customer.membership?.checkouts.length && <p className="dashboard-empty">No paid online membership enrollments or renewals are recorded.</p>}
      </section>
