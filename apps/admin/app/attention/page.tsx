...
type AttentionInbox = {
  boxOfficeOrders: Array<{ id: string; orderNumber: string; totalCents: number; currency: string; guestName: string | null; guestEmail: string | null; updatedAt: string }>;
  restaurantTabs: Array<{ id: string; label: string | null; status: string; totalCents: number | null; updatedAt: string; location: { currency: string }; primaryCustomer: { name: string | null; email: string | null } | null; showtime: { movie: { title: string }; auditorium: { name: string } } | null }>;
  ...
  return <main className="admin-route-page"><section className="panel attention-inbox"><p className="kicker">ACTION REQUIRED</p><h2>Manager attention</h2><p>Operational exceptions that need a person to follow up. Revenue and performance metrics remain on the Dashboard and Reports pages.</p>
    ...
    {inbox && <><div className={count ? "attention-summary" : "status-chip"}>{count ? `${count} items need attention` : "No attention items"}</div>
      <div className="attention-sections">
        <section><h3>Box-office payment failures <span>{inbox.boxOfficeOrders.length}</span></h3>{inbox.boxOfficeOrders.map((order) => <article key={order.id}><div><strong>{order.orderNumber}</strong><small>{order.guestName ?? "Guest"} · {order.guestEmail ?? "No email"} · {when(order.updatedAt)}</small></div><strong>{money(order.totalCents, order.currency)}</strong><Link href={`/refunds?query=${encodeURIComponent(order.orderNumber)}`}>Resolve</Link></article>)}{!inbox.boxOfficeOrders.length && <p className="dashboard-empty">No failed box-office orders.</p>}</section>
        <section><h3>Restaurant settlement <span>{inbox.restaurantTabs.length}</span></h3>{inbox.restaurantTabs.map((tab) => <article key={tab.id}><div><strong>{tab.label ?? tab.showtime?.movie.title ?? "Restaurant tab"}</strong><small>{tab.primaryCustomer?.name ?? tab.primaryCustomer?.email ?? tab.showtime?.auditorium.name ?? "Guest"} · {tab.status.replaceAll("_", " ")} · {when(tab.updatedAt)}</small></div>{tab.totalCents != null && <strong>{money(tab.totalCents, tab.location.currency)}</strong>}<Link href="/refunds">Resolve</Link></article>)}{!inbox.restaurantTabs.length && <p className="dashboard-empty">No restaurant checks need review.</p>}</section>
