...
type AttentionInbox = {
  ...
  failedRefunds: Array<{ id: string; amountCents: number; reason: string; scope: string; updatedAt: string; payment: { currency: string; ticketOrder: { orderNumber: string; customer: { id: string } | null } | null; restaurantTab: { id: string; label: string | null; primaryCustomer: { id: string } | null } | null } }>;
...
const failedRefundCustomerId = (refund: AttentionInbox["failedRefunds"][number]) =>
  refund.payment.ticketOrder?.customer?.id ??
  refund.payment.restaurantTab?.primaryCustomer?.id ??
  null;
  ...
  return <main className="admin-route-page"><section className="panel attention-inbox"><p className="kicker">ACTION REQUIRED</p><h2>Manager attention</h2><p>Operational exceptions that need a person to follow up. Revenue and performance metrics remain on the Dashboard and Reports pages.</p>
    ...
    {inbox && <><div className={count ? "attention-summary" : "status-chip"}>{count ? `${count} items need attention` : "No attention items"}</div>
      <div className="attention-sections">
        ...
        <section><h3>Failed refunds <span>{inbox.failedRefunds.length}</span></h3>{inbox.failedRefunds.map((refund) => <article key={refund.id}><div><strong>{refund.payment.ticketOrder?.orderNumber ?? refund.payment.restaurantTab?.label ?? "Restaurant refund"}</strong><small>{refund.scope} · {refund.reason} · {when(refund.updatedAt)}</small></div><strong>{money(refund.amountCents, refund.payment.currency)}</strong><div className="search-result-actions">{failedRefundCustomerId(refund) && <Link href={`/customers/${encodeURIComponent(failedRefundCustomerId(refund)!)}`}>Open customer</Link>}<Link href={refund.payment.ticketOrder ? `/refunds?query=${encodeURIComponent(refund.payment.ticketOrder.orderNumber)}` : "/refunds"}>Retry</Link></div></article>)}{!inbox.failedRefunds.length && <p className="dashboard-empty">No failed refunds.</p>}</section>
