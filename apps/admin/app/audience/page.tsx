...
type Report = { generatedAt: string; range: { from: string; to: string }; totals: Counts & Rates; daily: Array<{ date: string } & Counts>; pages: Array<{ path: string; count: number }> };
...
export default function AudiencePage() {
  ...
  return <main className="admin-shell audience-analytics-page">
    ...
    {report && <>
      ...
      <section className="audience-funnel" aria-labelledby="checkout-funnel-heading"><h2 id="checkout-funnel-heading">Ticket checkout funnel</h2><div><article><span>Seats continued</span><strong>{report.totals["Seat Selection Continued"]}</strong><small>continued from seat selection</small></article><article><span>Checkout created</span><strong>{report.totals["Checkout Started"]}</strong><small>{rate(report.totals.seatToCheckoutRatePercent)} from seats</small></article><article><span>Payment ready</span><strong>{report.totals["Payment Form Ready"]}</strong><small>{rate(report.totals.paymentFormReadyRatePercent)} from checkout</small></article><article><span>Completed</span><strong>{report.totals["Checkout Completed"]}</strong><small>{rate(report.totals.paymentCompletionRatePercent)} from payment</small></article></div></section>
      <section className="audience-funnel"><h2>Other customer actions</h2><div><article><span>Gift cards purchased</span><strong>{report.totals["Gift Card Purchased"]}</strong><small>{rate(report.totals.giftCardCompletionRatePercent)} completion</small></article><article><span>Memberships activated</span><strong>{report.totals["Membership Activated"]}</strong><small>{rate(report.totals.membershipCompletionRatePercent)} completion</small></article><article><span>Donations completed</span><strong>{report.totals["Donation Completed"]}</strong><small>{rate(report.totals.donationCompletionRatePercent)} completion</small></article><article><span>Private-event inquiries</span><strong>{report.totals["Private Event Inquiry Submitted"]}</strong><small>submitted inquiries</small></article></div></section>
