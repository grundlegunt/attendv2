...
type Counts = Record<"Pageview" | "Acquisition Source" | "Seat Selection Continued" | "Checkout Started" | "Payment Form Ready" | "Checkout Completed" | "Account Created" | "Gift Card Started" | "Gift Card Purchased" | "Membership Checkout Started" | "Membership Activated" | "Donation Checkout Started" | "Donation Completed" | "Private Event Inquiry Submitted" | "Waitlist Joined", number>;
...
type Report = { generatedAt: string; range: { from: string; to: string }; totals: Counts & Rates; comparison: { range: { from: string; to: string }; totals: Counts & Rates }; daily: Array<{ date: string } & Counts>; pages: Array<{ path: string; count: number }>; sources: Array<{ source: string; count: number }> };
...
export default function AudiencePage() {
  ...
  return <main className="admin-shell audience-analytics-page">
    ...
    {report && <>
      ...
      <section className="panel audience-sources"><p className="kicker">ACQUISITION</p><h2>How customers arrived</h2><p>Session-level categories from consenting customers. Raw referral URLs and campaign names are not stored.</p><div className="audience-pages">{report.sources.length === 0 && <p className="dashboard-empty">No consented acquisition activity in this range.</p>}{report.sources.map((source) => <div key={source.source}><strong>{source.source}</strong><strong>{source.count.toLocaleString()}</strong></div>)}</div></section>
