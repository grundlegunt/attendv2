...
export default function DiagnosticsPage() {
  ...
  return <main className="admin-shell diagnostics-page">
    ...
    <section className="panel diagnostics-explanation"><strong>How to read this</strong><p>High database time or query count identifies database-heavy endpoints. High API time with low database time points to application work. High outside-API time points to network, cold-start connection overhead, transfer, or browser processing. Response size is shown when the server reports it.</p></section>
    <section className="diagnostics-table" aria-label="Recent Admin API requests"><div className="diagnostics-row diagnostics-header"><strong>Admin page / endpoint</strong><span>Status</span><span>Browser wait</span><span>API time</span><span>Database</span><span>Queries</span><span>Outside API</span><span>Size</span><span>Recorded</span></div>{timings.map((timing, index) => <article className={`diagnostics-row ${timing.totalMs >= 1_000 ? "slow" : ""}`} key={`${timing.recordedAt}-${index}`}><strong><small>{timing.page}</small>{timing.method} {timing.path}</strong><span>{timing.status ?? "Network error"}</span><span>{duration(timing.totalMs)}</span><span>{duration(timing.serverMs)}</span><span>{duration(timing.databaseMs)}</span><span>{timing.databaseQueryCount ?? "Unavailable"}</span><span>{duration(timing.serverMs === null ? null : Math.max(0, timing.totalMs - timing.serverMs))}</span><span>{bytes(timing.responseBytes)}</span><time>{new Date(timing.recordedAt).toLocaleTimeString()}</time></article>)}</section>
