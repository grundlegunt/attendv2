...
type DeliveryReadiness = Record<"email" | "sms" | "appleWallet" | "googleWallet", { ready: boolean; provider: string }>;
...
export default function DiagnosticsPage() {
  ...
  useEffect(() => {
    let active = true;
    apiFetch<DeliveryReadiness>("/cinema/admin/delivery-readiness", { accessToken })
      .then((value) => { if (active) setDelivery(value); })
      .catch(() => { if (active) setDeliveryError(true); });
    return () => { active = false; };
  }, [accessToken]);
  ...
  return <main className="admin-shell diagnostics-page">
    ...
    <section className="panel delivery-readiness" aria-labelledby="delivery-readiness-heading"><div className="dashboard-section-heading"><div><p className="kicker">CUSTOMER DELIVERY</p><h2 id="delivery-readiness-heading">Ticket delivery readiness</h2><p>These platform-level providers control which receipt, text-message, and mobile-wallet options customers can use.</p></div></div>{delivery ? <div className="delivery-readiness-grid">{([['email', 'Email receipts'], ['sms', 'SMS tickets'], ['appleWallet', 'Apple Wallet'], ['googleWallet', 'Google Wallet']] as const).map(([key, label]) => <article key={key}><span>{label}</span><strong className={delivery[key].ready ? "ready" : "not-ready"}>{delivery[key].ready ? "Ready" : "Not configured"}</strong><small>Provider: {delivery[key].provider}</small></article>)}</div> : <p className={deliveryError ? "inline-error" : "dashboard-empty"}>{deliveryError ? "Delivery readiness could not be loaded." : "Checking delivery providers…"}</p>}</section>
