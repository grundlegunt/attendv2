...
type Performance = {
  ...
  fnbItems: Array<{ menuItemId: string; name: string; chargeCategory: "FOOD" | "ALCOHOL" | "NA_BEVERAGE"; unitsSold: number; salesCents: number; orderAheadUnits: number; serviceUnits: number }>;
  auditoriumPerformance: PerformanceSlice[];
  daypartPerformance: PerformanceSlice[];
  weekdayPerformance: PerformanceSlice[];
  ...
  dailyPerformance: Array<{ date: string; showtimes: number; ticketsSold: number; capacity: number; attendancePercent: number; averageTicketsPerShow: number; ticketRevenueCents: number; fnbRevenueCents: number; averageFnbPerShowCents: number; distributorRevenueCents: number; cinemaRevenueCents: number; unallocatedRevenueCents: number }>;
...
export default function FilmPerformancePage() {
  ...
  return <main className="admin-route-page series-performance-page film-performance-page">
    ...
    {performance && <>
      ...
      <section className="panel film-daily-performance"><div className="dashboard-section-heading"><div><p className="kicker">DAILY TREND</p><h2>Performance by cinema date</h2></div><span>{performance.dailyPerformance.length} active dates</span></div><div className="film-daily-table"><header><span>Date</span><span>Shows</span><span>Tickets / attendance</span><span>Ticket revenue</span><span>F&amp;B / average</span><span>Cinema / distributor</span></header>{performance.dailyPerformance.map((day) => <article key={day.date}><span><strong>{new Date(`${day.date}T12:00:00Z`).toLocaleDateString([], { timeZone: performance.location.timezone, weekday: "short", month: "short", day: "numeric", year: "numeric" })}</strong></span><span>{day.showtimes}<small>{day.averageTicketsPerShow} avg tickets</small></span><span>{day.ticketsSold}<small>{day.attendancePercent}% of {day.capacity}</small></span><span>{money(day.ticketRevenueCents, currency)}</span><span>{money(day.fnbRevenueCents, currency)}<small>{money(day.averageFnbPerShowCents, currency)} per show</small></span><span>{money(day.cinemaRevenueCents, currency)}<small>{money(day.distributorRevenueCents, currency)} distributor</small></span></article>)}</div>{performance.dailyPerformance.length === 0 && <p className="dashboard-empty">No performances in this period.</p>}</section>
      ...
      <section className="panel film-advance-sales"><div className="dashboard-section-heading"><div><p className="kicker">BOOKING CURVE</p><h2>How far ahead audiences buy</h2></div><span>{performance.totals.ticketsSold} total tickets</span></div><div className="film-advance-sales-list">{performance.advanceSales.map((bucket) => <article key={bucket.key}><div><strong>{bucket.label}</strong><small>{bucket.averageLeadHours < 48 ? `${bucket.averageLeadHours} average hours ahead` : `${Math.round(bucket.averageLeadHours / 24)} average days ahead`}</small></div><div className="film-advance-sales-bar"><i><span style={{ width: `${bucket.percentOfTickets}%` }} /></i><b>{bucket.percentOfTickets}%</b></div><span>{bucket.ticketsSold} tickets</span><span>{money(bucket.ticketRevenueCents, currency)}</span></article>)}{performance.advanceSales.length === 0 && <p className="dashboard-empty">No ticket purchases in this period.</p>}</div></section>
      <section className="panel film-fnb-items"><div className="dashboard-section-heading"><div><p className="kicker">CONCESSION MIX</p><h2>Food &amp; drink items sold</h2></div><span>{performance.fnbItems.reduce((sum, item) => sum + item.unitsSold, 0)} units</span></div><div className="film-fnb-item-table"><header><span>Item / category</span><span>Units</span><span>Ticket checkout</span><span>During show</span><span>Item sales</span></header>{performance.fnbItems.map((item) => <article key={item.menuItemId}><span><strong>{item.name}</strong><small>{item.chargeCategory === "NA_BEVERAGE" ? "Non-alcoholic beverage" : item.chargeCategory === "ALCOHOL" ? "Alcohol" : "Food"}</small></span><span>{item.unitsSold}</span><span>{item.orderAheadUnits}</span><span>{item.serviceUnits}</span><span>{money(item.salesCents, currency)}</span></article>)}</div>{performance.fnbItems.length === 0 && <p className="dashboard-empty">No food or drink items were sold for this film in this period.</p>}</section>
