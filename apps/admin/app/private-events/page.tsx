...
export default function PrivateEventsAdminPage() {
  const { accessToken, employee } = useAdminSession(); const timeZone = employee.timezone; const [items, setItems] = useState<Inquiry[]>([]); const [query, setQuery] = useState(""); const [statusFilter, setStatusFilter] = useState(""); const [error, setError] = useState<string | null>(null);
  ...
  async function status(id: string, next: string) {
    ...
    setError(null);
    const body = JSON.stringify({ status: next });
    ...
    try {
      await apiFetch(`/management/private-event-inquiries/${id}`, { accessToken, method: "PATCH", headers: { "Idempotency-Key": statusAttemptRef.current.requestId }, body });
      statusAttemptRef.current = null;
      setItems((current) => current.map((item) => item.id === id ? { ...item, status: next } : item).filter((item) => !statusFilter || item.status === statusFilter));
    } catch (reason) {
      if (reason instanceof ApiRequestError && reason.status < 500) statusAttemptRef.current = null;
      setError(reason instanceof ApiRequestError ? reason.body.message : "The inquiry status could not be updated.");
    } finally {
