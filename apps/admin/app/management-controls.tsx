...
export function ManagementControls({ accessToken, permissions, section }: { accessToken: string; permissions: string[]; section: ControlSection }) {
  ...
  async function createTicketType(event: FormEvent) {
    event.preventDefault(); setError(null);
    try { await apiFetch("/management/settings/ticket-types", { accessToken, method: "POST", body: JSON.stringify({ name: newTicketTypeName }) }); setNewTicketTypeName(""); await refresh(); } catch (reason) { showError(reason); }
  }
  async function updateTicketType(ticketType: Settings["ticketTypes"][number], changes: { name?: string; active?: boolean }) {
    setError(null);
