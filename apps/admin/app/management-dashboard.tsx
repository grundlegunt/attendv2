...
type RevenueReport = {
  ...
  salesChannels: Array<{ channel: "ONLINE" | "BOX_OFFICE"; ticketsSold: number; ticketRevenueCents: number; collectedCents: number }>;
};
...
export function ManagementDashboard({ accessToken, permissions, section }: { accessToken: string; permissions: string[]; section: ManagementSection }) {
  ...
  return <section className="management-stack">
    ...
    {revenue && <section className="panel"><p className="kicker">FINANCE</p><h2>Revenue overview</h2>
      ...
      <h3>By sales channel</h3><div className="management-table"><div className="table-row table-head"><span>Channel</span><span>Tickets</span><span>Ticket face value</span><span>Total collected</span></div>{revenue.salesChannels.map((row) => <div className="table-row" key={row.channel}><strong>{row.channel === "BOX_OFFICE" ? "Box office" : "Online"}</strong><span>{row.ticketsSold}</span><span>{money(row.ticketRevenueCents)}</span><span>{money(row.collectedCents)}</span></div>)}</div>
    </section>}
