...
type RevenueReport = {
  totals: { grossRevenueCents: number; refundedCents: number; ticketRefundedCents: number; fnbRefundedCents: number; ticketRevenueCents: number; ticketFeesCents: number; ticketTaxCents: number; ticketCollectedCents: number; distributorRevenueCents: number; cinemaFilmRevenueCents: number; unallocatedFilmRevenueCents: number; fnbRevenueCents: number; combinedRevenueCents: number; ticketsSold: number; fnbOrders: number; averageFnbSpendPerOrderCents: number; averageFnbSpendPerSeatCents: number; averageTotalSpendPerPatronCents: number; concessionAttachRatePercent: number };
  movies: Array<{ movieId: string; title: string; distributorName: string | null; ticketRevenueCents: number; ticketsSold: number; fnbRevenueCents: number; distributorRevenueCents: number; cinemaRevenueCents: number; unallocatedRevenueCents: number; allocationComplete: boolean }>;
  ...
  ticketFeeDetails: Array<{ orderId: string; orderNumber: string; createdAt: string; channel: "ONLINE" | "BOX_OFFICE"; ticketsSold: number; ticketFeesCents: number; averageFeeCents: number; movieTitles: string[] }>;
...
type OperatingSettings = { name: string; address: string | null; timezone: string; currency: string; timeClockEnabled: boolean; ticketTaxRateBasisPoints: number; ticketFeesTaxable: boolean; preShowBufferMinutes: number; cleaningBufferMinutes: number; checkDropMinutesBeforeEnd: number; autoSettleGraceMinutes: number; autoSettleTipBasisPoints: number };
...
export function ManagementDashboard({ accessToken, permissions, section, timeZone }: { accessToken: string; permissions: string[]; section: ManagementSection; timeZone: string }) {
  ...
  return <section className="management-stack">
    ...
    {revenue && <section className="panel"><p className="kicker">FINANCE</p><h2>Revenue overview</h2>
      ...
      <h3>Film rental allocation</h3><p>Calculated from ticket face value using each film's theatrical-week deal terms. Fees, tax, refunds, and food are excluded.</p><div className="management-table"><div className="table-row table-head film-allocation-row"><span>Movie</span><span>Tickets</span><span>Face value</span><span>Distributor</span><span>Cinema</span><span>Status</span></div>{revenue.movies.map((row) => <div className="table-row film-allocation-row report-movie-row" id={`movie-${row.movieId}`} key={row.movieId}><strong><Link href={`/films/${encodeURIComponent(row.movieId)}`}>{row.title}</Link><small>{row.distributorName ?? "Distributor not set"}</small></strong><span>{row.ticketsSold}</span><span>{money(row.ticketRevenueCents)}</span><span>{money(row.distributorRevenueCents)}</span><span>{money(row.cinemaRevenueCents)}</span><span>{row.allocationComplete ? "Allocated" : `${money(row.unallocatedRevenueCents)} needs terms`}</span></div>)}</div>
      ...
      <details className="report-drilldown"><summary>View ticket-fee order details ({revenue.ticketFeeDetails.length})</summary><div className="management-table"><div className="table-row table-head"><span>Order</span><span>Channel</span><span>Tickets</span><span>Fees</span></div>{revenue.ticketFeeDetails.map((row) => <div className="table-row" key={row.orderId}><strong>{row.orderNumber}<small>{cinemaDateTime(row.createdAt, timeZone)} · {row.movieTitles.join(", ")}</small></strong><span>{row.channel === "BOX_OFFICE" ? "Box office" : "Online"}</span><span>{row.ticketsSold}<small>{money(row.averageFeeCents)} each</small></span><span>{money(row.ticketFeesCents)}</span></div>)}</div>{revenue.ticketFeeDetails.length === 0 && <p className="dashboard-empty">No ticket fees were collected in this range.</p>}</details>
    ...
    {locationDraft && section === "location" && <form className="panel location-settings" onSubmit={(event) => void saveLocation(event)}><p className="kicker">LOCATION</p><h2>Operating settings</h2><p>These values control the public venue identity, scheduling turnover, dining settlement, and staff time clock.</p>
      <div className="location-settings-grid">
        ...
        <label>Cleaning buffer (minutes)<input type="number" min="15" max="240" required value={locationDraft.cleaningBufferMinutes} onChange={(event) => setLocationDraft({ ...locationDraft, cleaningBufferMinutes: Number(event.target.value) })} /><small>Ringo enforces at least 15 minutes.</small></label>
      ...
      <label className="checkbox"><input type="checkbox" checked={locationDraft.ticketFeesTaxable} onChange={(event) => setLocationDraft({ ...locationDraft, ticketFeesTaxable: event.target.checked })} /> Apply ticket tax to customer service and booking fees</label>
      <small>Enable this only when the location’s jurisdiction treats ticket fees as taxable.</small>
