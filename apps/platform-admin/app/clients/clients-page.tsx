...
export default function AttendMaster() {
  ...
  const [ticketFeeReconciliationNote, setTicketFeeReconciliationNote] = useState("");
  ...
  async function finalizeTicketFeeRemittance() {
    ...
    if (organization.ticketFeeSettlement.varianceCents !== 0 && !ticketFeeReconciliationNote.trim()) {
      setError("Add a reconciliation note explaining the settlement variance before finalizing this period.");
      return;
    }
    ...
    try {
      await request(`/platform/organizations/${organization.id}/ticket-fee-remittances`, {
        ...
        body: JSON.stringify({
          ...
          reconciliationNote: ticketFeeReconciliationNote.trim() || null,
        }),
      }, session.accessToken);
      setTicketFeeReconciliationNote("");
            ...
            <>
              ...
              <section className="dashboard-panel commercial-terms-panel">
                ...
                {organization.ticketFeeSettlement && <div className="agreement-settlement">
                  <div><small>Net paid tickets</small><strong>{organization.ticketFeeSettlement.tickets.toLocaleString()}</strong><em>{utcCalendarDate(organization.ticketFeeSettlement.periodFrom)} – {organization.ticketFeeSettlement.periodTo ? utcCalendarDate(organization.ticketFeeSettlement.periodTo) : "lifetime"}</em></div>
                  <div><small>Fees collected</small><strong>{money(organization.ticketFeeSettlement.collectedFeeCents)}</strong><em>Actual checkout service fees</em></div>
                  <div><small>Agreement expected fees</small><strong>{money(organization.ticketFeeSettlement.platformShareCents + organization.ticketFeeSettlement.operatorShareCents)}</strong><em>{organization.ticketFeeSettlement.varianceCents === 0 ? "Reconciled to agreement" : `${money(organization.ticketFeeSettlement.varianceCents)} collected variance`}</em></div>
                ...
                {organization.ticketFeeSettlement && organization.ticketFeeSettlement.varianceCents !== 0 && <div className="agreement-fee-warning agreement-settlement-variance" role="alert"><div><strong>Settlement needs reconciliation</strong><p>Actual checkout fees differ from the agreement&apos;s expected fees by {money(organization.ticketFeeSettlement.varianceCents)}. Review guest and registered-customer pricing, refunds, and the agreement basis before finalizing this period.</p></div></div>}
