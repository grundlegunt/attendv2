...
type Report = {
  ...
  sourcesByDay: Array<{ date: string; sources: Array<{ source: string; count: number }> }>;
  locations: Array<{ organization: { id: string; name: string }; location: { id: string; name: string }; sources: Array<{ source: string; count: number }> } & Counts & Rates>;
};
...
export default function AudienceAnalyticsPage() {
  ...
  return <main className="shell">
    ...
    {report && <>
      ...
      <section className="dashboard-panel"><div className="panel-heading"><div><p className="eyebrow">ACQUISITION</p><h2>How customers arrived</h2><p className="muted">Privacy-safe categories reported once per consenting browser session.</p></div></div><div className="analytics-page-list">{report.sources.length === 0 && <p className="empty-state">No consented acquisition activity in this range.</p>}{report.sources.map((source) => <div key={source.source}><strong>{source.source}</strong><strong>{source.count.toLocaleString()}</strong></div>)}</div></section>
