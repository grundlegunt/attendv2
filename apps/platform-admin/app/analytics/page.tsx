...
type Report = {
  ...
  sources: Array<{ source: string; count: number; sharePercent: number }>;
  sourcesByDay: Array<{ date: string; sources: Array<{ source: string; count: number; sharePercent: number }> }>;
  locations: Array<{ organization: { id: string; name: string }; location: { id: string; name: string }; sources: Array<{ source: string; count: number; sharePercent: number }> } & Counts & Rates>;
};
...
export default function AudienceAnalyticsPage() {
  ...
  return <main className="shell">
    ...
    {report && <>
      ...
      <section className="dashboard-panel analytics-trend"><div className="panel-heading"><div><p className="eyebrow">TREND</p><h2>Daily customer activity</h2><p className="muted">Every selected day is shown, including dates with no consented activity.</p></div></div><div className="analytics-trend-list">{report.daily.map((day) => { const activity = day.Pageview + day["Checkout Started"] + day["Checkout Completed"]; return <div key={day.date}><time dateTime={day.date}>{new Date(`${day.date}T12:00:00`).toLocaleDateString(undefined, { month: "short", day: "numeric" })}</time><span className="analytics-trend-bar"><i style={{ width: `${activity / maxDailyActivity * 100}%` }} /></span><strong>{day.Pageview}<small>views</small></strong><strong>{day["Checkout Started"]}<small>starts</small></strong><strong>{day["Checkout Completed"]}<small>sales</small></strong></div>; })}</div></section>
      <div className="dashboard-grid analytics-grid"><section className="dashboard-panel"><div className="panel-heading"><div><p className="eyebrow">CLIENTS</p><h2>Engagement by location</h2><p className="muted">Every active cinema is listed, including locations with no consented activity.</p></div></div><div className="analytics-location-list"><div><strong>Client / location</strong><span>Views</span><span>Checkout</span><span>Members</span><span>Donations</span><span>Gift cards</span></div>{report.locations.map((row) => <Link key={row.location.id} href={`/clients?organizationId=${encodeURIComponent(row.organization.id)}&locationId=${encodeURIComponent(row.location.id)}`}><strong>{row.organization.name}<small>{row.location.name}</small></strong><span>{row.Pageview}</span><span>{rate(row.checkoutCompletionRatePercent)}</span><span>{row["Membership Activated"]}</span><span>{row["Donation Completed"]}</span><span>{row["Gift Card Purchased"]}</span></Link>)}</div></section><section className="dashboard-panel"><div className="panel-heading"><div><p className="eyebrow">CONTENT</p><h2>Top pages</h2></div></div><div className="analytics-page-list">{report.pages.length === 0 && <p className="empty-state">No consented pageviews in this range.</p>}{report.pages.map((page) => <div key={page.path}><code>{page.path}</code><strong>{page.count.toLocaleString()}</strong></div>)}</div></section></div>
      <section className="dashboard-panel"><div className="panel-heading"><div><p className="eyebrow">ACQUISITION</p><h2>How customers arrived</h2><p className="muted">Privacy-safe categories reported once per consenting browser session.</p></div></div><div className="analytics-page-list">{report.sources.length === 0 && <p className="empty-state">No consented acquisition activity in this range.</p>}{report.sources.map((source) => <div key={source.source}><strong>{source.source}</strong><span>{source.sharePercent.toFixed(2)}%</span><strong>{source.count.toLocaleString()}</strong></div>)}</div></section>
      <section className="dashboard-panel"><div className="panel-heading"><div><p className="eyebrow">ACQUISITION TREND</p><h2>Leading source by day</h2><p className="muted">Daily leaders help separate sustained discovery from short campaign spikes.</p></div></div><div className="analytics-page-list">{report.sourcesByDay.length === 0 && <p className="empty-state">No consented acquisition activity in this range.</p>}{report.sourcesByDay.map((day) => { const [source] = day.sources; return source ? <div key={day.date}><time dateTime={day.date}>{new Date(`${day.date}T12:00:00`).toLocaleDateString(undefined, { month: "short", day: "numeric", year: "numeric" })}</time><span>{source.source}</span><strong>{source.count.toLocaleString()}</strong></div> : null; })}</div></section>
      {!organizationId && <section className="dashboard-panel"><div className="panel-heading"><div><p className="eyebrow">CLIENT COMPARISON</p><h2>Leading acquisition source by location</h2><p className="muted">Use this directional comparison to see which channel introduces the most consenting sessions at each cinema.</p></div></div><div className="analytics-page-list">{report.locations.every((location) => location.sources.length === 0) && <p className="empty-state">No consented acquisition activity by location in this range.</p>}{report.locations.map((location) => { const [source] = location.sources; return source ? <div key={location.location.id}><strong>{location.organization.name}<small>{location.location.name}</small></strong><span>{source.source} · {source.sharePercent.toFixed(2)}%</span><strong>{source.count.toLocaleString()}</strong></div> : null; })}</div></section>}
