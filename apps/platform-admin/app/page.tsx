    ...
    <section className="organizations">
      ...
      {overview?.organizations.map((organizationItem) => <article className="organization" key={organizationItem.id}>
        ...
        <div className="location-list">{organizationItem.locations.map((location) => <div className="location" key={location.id}>
          ...
          <div className="actions"><a href={CINEMA_ADMIN_URL} target="_blank" rel="noreferrer">Cinema admin ↗</a><a href={`${CUSTOMER_WEB_URL.replace(/\/$/, "")}/showtimes?locationId=${encodeURIComponent(location.id)}`} target="_blank" rel="noreferrer">Customer site ↗</a></div>
        </div>)}</div>
      </article>)}
    </section>
    </>}
