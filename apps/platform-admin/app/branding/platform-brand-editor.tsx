...
export function PlatformBrandEditor({ accessToken, canEdit, request }: { accessToken: string; canEdit: boolean; request: <T>(path: string, init?: RequestInit, accessToken?: string) => Promise<T> }) {
  ...
  return <section className="platform-brand-editor">
    ...
    <div className="platform-brand-fields">
      ...
      <fieldset><legend>Customer analytics</legend><label className="brand-analytics-toggle"><input type="checkbox" checked={brand.analytics.enabled} disabled={!canEdit} onChange={(event) => setBrand((current) => ({ ...current, analytics: { ...current.analytics, enabled: event.target.checked } }))} /><span>Enable privacy-consented Plausible analytics on customer websites</span></label><p className="muted">Uses Plausible’s fixed HTTPS script. Tracking remains off until each visitor explicitly allows optional analytics; customer and order identifiers are never sent.</p></fieldset>
    </div>
