...
export default function PlatformTeam() {
  ...
  return <main className="shell">
    ...
    <nav className="platform-nav" aria-label="Attend Master"><Link href="/">Dashboard</Link><Link href="/clients">Clients</Link><Link href="/onboarding">Onboarding</Link><Link href="/payments">Payments</Link><Link href="/content">Content</Link><Link className="active" href="/team">Team</Link><Link href="/audit">Audit Log</Link></nav>
    ...
    <section className="team-layout">
      ...
      <section className="team-roster"><div className="section-heading"><div><p className="eyebrow">COMPANY ACCESS</p><h2>{users.filter((user) => user.active).length} active operator{users.filter((user) => user.active).length === 1 ? "" : "s"}</h2></div></div>{users.map((user) => <article key={user.id}><div className="team-operator"><strong>{user.name}{user.id === session.user.id ? " (you)" : ""}</strong><span>{user.email}</span><small>Added {new Date(user.createdAt).toLocaleDateString()}</small><div className="team-password-reset"><label>Temporary password<input type="password" minLength={12} maxLength={200} value={credentialDrafts[user.id] ?? ""} onChange={(event) => setCredentialDrafts((current) => ({ ...current, [user.id]: event.target.value }))} /></label><button className="quiet" disabled={workingId === user.id || (credentialDrafts[user.id] ?? "").length < 12} onClick={() => void resetPassword(user)}>Reset password</button></div></div><div><span className={user.active ? "status good" : "status warning"}>{user.active ? "Active" : "Inactive"}</span><button className="quiet" disabled={workingId === user.id || user.id === session.user.id} onClick={() => void setAccess(user)}>{workingId === user.id ? "Saving…" : user.active ? "Deactivate" : "Reactivate"}</button></div></article>)}</section>
    </section>
  </main>;
}
