...
function feeVarianceLabel(cents: number) {
  if (cents === 0) return "Reconciled";
  return `${money(Math.abs(cents))} ${cents > 0 ? "over-collected" : "under-collected"}`;
}
...
export default function PlatformPayments() {
  ...
  useEffect(() => {
    ...
    if (owner) setRemittanceOwnerFilter(owner);
  ...
  async function assignFilteredRemittancesToMe() {
    if (!session || assignableDisplayedRemittances.length === 0) return;
    if (!window.confirm(`Assign ${assignableDisplayedRemittances.length} open remittance period${assignableDisplayedRemittances.length === 1 ? "" : "s"} to you?`)) return;
    setAssigningRemittances(true);
    setError(null);
    let failed = 0;
    for (const remittance of assignableDisplayedRemittances) {
      try {
    ...
    try {
      await loadOverview(session);
      if (failed > 0) setError(`${failed} of ${assignableDisplayedRemittances.length} remittances could not be assigned. Refresh and try those periods again.`);
    } catch (reason) {
      setError(reason instanceof Error ? reason.message : "Assignments were submitted, but the updated ledger could not be loaded.");
    } finally {
      setAssigningRemittances(false);
    }
  }
  ...
  function exportAgedReceivables() {
    const columns = ["Operator", "Period start", "Period end", "Tickets", "Fees collected", "Ringo receivable", "Operator share", "Variance", "Status", "Aging bucket", "Days overdue", "Due date", "Paid date", "Payment reference", "Collection owner", "Collection owner email", "Reconciliation note", "Collection notes", "Last contacted", "Next follow-up"];
  ...
  return (
    <main className="shell">
      ...
      <section className="remittance-master-table">
        ...
        {displayedRemittances.map((remittance) => <article key={remittance.id}><strong>{remittance.organizationName}<small>Owner: {remittance.collectionOwner?.name ?? "Unassigned"}</small></strong><span>{new Date(remittance.periodFrom).toLocaleDateString()} – {new Date(remittance.periodTo).toLocaleDateString()}</span><span>{remittance.ticketCount.toLocaleString()}</span><span><strong>{money(remittance.platformShareCents)}</strong><small>{money(remittance.collectedFeeCents)} collected · {money(remittance.platformShareCents + remittance.operatorShareCents)} expected</small><small className={remittance.varianceCents !== 0 ? "status warning" : "status good"}>{feeVarianceLabel(remittance.varianceCents)}</small></span><span><b className={`status ${remittance.status === "PAID" ? "good" : remittanceAgingBucket(remittance) !== "CURRENT" ? "warning" : ""}`}>{remittanceAgeLabel(remittance)}</b><small>{remittance.status === "PAID" && remittance.paidAt ? `Paid ${new Date(remittance.paidAt).toLocaleDateString()}` : remittance.dueDate ? `Due ${new Date(remittance.dueDate).toLocaleDateString()}` : "No due date"}</small>{remittance.reconciliationNote && <small title={remittance.reconciliationNote}>Reconciliation: {remittance.reconciliationNote}</small>}{remittance.notes && <small title={remittance.notes}>Collection: {remittance.notes}</small>}{remittance.nextFollowUpAt && <small className={remittance.status === "DUE" && new Date(remittance.nextFollowUpAt) < new Date() ? "status warning" : ""}>Follow up {new Date(remittance.nextFollowUpAt).toLocaleDateString()}</small>}</span><Link href={`/clients?organizationId=${encodeURIComponent(remittance.organizationId)}`}>Open client →</Link></article>)}
      </section>
