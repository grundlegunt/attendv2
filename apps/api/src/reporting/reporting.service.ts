...
export class ReportingService {
  ...
  async distributorPerformance(locationId: string, distributorName?: string, range?: ReportRange) {
    ...
    const distributorRows = new Map<string, { name: string; films: typeof filmRows; showtimes: number; ticketsSold: number; totalCapacity: number; attendancePercent: number; ticketRevenueCents: number; fnbRevenueCents: number; distributorRevenueCents: number; cinemaRevenueCents: number; unallocatedRevenueCents: number }>();
    for (const film of filmRows) {
      const key = film.distributorName.toLocaleLowerCase();
      const row = distributorRows.get(key) ?? { name: film.distributorName, films: [], showtimes: 0, ticketsSold: 0, totalCapacity: 0, attendancePercent: 0, ticketRevenueCents: 0, fnbRevenueCents: 0, distributorRevenueCents: 0, cinemaRevenueCents: 0, unallocatedRevenueCents: 0 };
      row.films.push(film); row.showtimes += film.showtimes; row.ticketsSold += film.ticketsSold; row.totalCapacity += film.totalCapacity; row.attendancePercent = row.totalCapacity ? Math.round((row.ticketsSold / row.totalCapacity) * 1000) / 10 : 0; row.ticketRevenueCents += film.ticketRevenueCents; row.fnbRevenueCents += film.fnbRevenueCents; row.distributorRevenueCents += film.distributorRevenueCents; row.cinemaRevenueCents += film.cinemaRevenueCents; row.unallocatedRevenueCents += film.unallocatedRevenueCents;
