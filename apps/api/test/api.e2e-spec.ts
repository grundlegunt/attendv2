...
describe("Milestone 1 cinema configuration", () => {
  ...
  it("retries a failed online gift card delivery without issuing another card", async () => {
    const { prisma } = await import("@cinema/database");
    const owner = await prisma.employee.findFirstOrThrow({ where: { email: `owner@${SEED_SUFFIX}` } });
    const idempotencyKey = `gift-card-delivery-retry-${crypto.randomUUID()}`;
    const purchase = await request(app.getHttpServer()).post("/api/v1/gift-card-purchases")
      .set("Idempotency-Key", idempotencyKey).send({
        locationId: owner.locationId, amountCents: 5100, buyerEmail: "retry-buyer@example.test",
    ...
    expect(await prisma.giftCard.count({ where: { purchase: { id: purchase.body.purchaseId } } })).toBe(1);
    expect(await prisma.giftCardPurchase.findUniqueOrThrow({ where: { id: purchase.body.purchaseId } })).toMatchObject({ status: "DELIVERED", deliveryCodeEncrypted: null, deliveredAt: expect.any(Date) });
...
describe("Milestone 3 ticket checkout and payment recovery", () => {
  ...
  async function createReceiptDeliveryFixture(label: string, orderId?: string) {
    const { prisma } = await import("@cinema/database");
    ...
    await prisma.ticketOrder.update({
      ...
      data: {
        guestEmail: `receipt-claim-${label}-${crypto.randomUUID()}@example.test`,
        receiptEmailSentAt: null,
        receiptEmailMessageId: null,
        receiptEmailClaimedAt: null,
        receiptEmailError: "retry receipt delivery",
  ...
  it("does not let a stale ticket receipt worker overwrite a newer successful delivery", async () => {
    const { prisma } = await import("@cinema/database");
    const { TicketingService } = await import("../src/ticketing/ticketing.service");
    const { EMAIL_PROVIDER } = await import("../src/notifications/notifications.module");
    const order = await createReceiptDeliveryFixture("success");
    const ticketing = app.get(TicketingService) as unknown as {
      domain: { deliverReceipt(input: typeof order): Promise<"SENT" | "FAILED" | "NOT_REQUESTED"> };
    };
    const emailProvider = app.get(EMAIL_PROVIDER) as TestEmailProvider;
    let releaseFirst!: () => void;
    let markFirstStarted!: () => void;
    ...
    const send = jest.spyOn(emailProvider, "sendTicketReceipt").mockImplementation(async () => {
      ...
      if (calls === 1) {
        markFirstStarted();
        await firstBlocked;
        return { messageId: "stale-ticket-receipt" };
      }
      return { messageId: "current-ticket-receipt" };
    });
    try {
      const staleWorker = ticketing.domain.deliverReceipt(order);
      await firstStarted;
      await prisma.ticketOrder.update({
        where: { id: order.id },
        data: { receiptEmailClaimedAt: new Date(Date.now() - 6 * 60_000) },
      });
      const currentOrder = await createReceiptDeliveryFixture("replacement", order.id);
      await expect(ticketing.domain.deliverReceipt(currentOrder)).resolves.toBe("SENT");
      releaseFirst();
      await expect(staleWorker).resolves.toBe("NOT_REQUESTED");
      await expect(prisma.ticketOrder.findUniqueOrThrow({ where: { id: order.id } })).resolves.toMatchObject({
        receiptEmailMessageId: "current-ticket-receipt",
        receiptEmailSentAt: expect.any(Date),
        receiptEmailClaimedAt: null,
        receiptEmailError: null,
      });
  ...
  it("does not let a stale failing ticket receipt worker clear a newer delivery claim", async () => {
    const { prisma } = await import("@cinema/database");
    const { TicketingService } = await import("../src/ticketing/ticketing.service");
    const { EMAIL_PROVIDER } = await import("../src/notifications/notifications.module");
    const order = await createReceiptDeliveryFixture("failure");
    const ticketing = app.get(TicketingService) as unknown as {
      domain: { deliverReceipt(input: typeof order): Promise<"SENT" | "FAILED" | "NOT_REQUESTED"> };
    };
    const emailProvider = app.get(EMAIL_PROVIDER) as TestEmailProvider;
    ...
    const send = jest.spyOn(emailProvider, "sendTicketReceipt").mockImplementation(async () => {
      markSendStarted();
      await sendBlocked;
      throw new Error("stale ticket receipt failed");
    });
    const newerClaim = new Date(Date.now() + 1_000);
    try {
      const staleWorker = ticketing.domain.deliverReceipt(order);
...
describe("Milestone 9 box office and workforce", () => {
  ...
  it("uses shared seat inventory for a mixed-tender box-office sale and full refund", async () => {
    ...
    const refundAttempts = await Promise.all(refundRequestIds.map((requestId) =>
      request(app.getHttpServer()).post(`/api/v1/box-office/orders/${sale.body.id}/refund`)
        .set("Authorization", `Bearer ${ownerAccessToken}`).send({ requestId, reason: "E2E full refund", cashDrawerId: drawer.body.id }),
    ));
    ...
    expect(await prisma.refund.count({ where: { paymentId: sale.body.payment.id } })).toBe(1);
  ...
  it("keeps a box-office seat sold until its pending card refund succeeds", async () => {
    ...
    const drawer = await request(app.getHttpServer()).post("/api/v1/box-office/cash-drawers")
      .set("Authorization", `Bearer ${ownerLogin.body.accessToken}`)
      .send({ requestId: crypto.randomUUID(), registerId: `PENDING-${crypto.randomUUID()}`, openingBalanceCents: 20_000 })
      .expect(201);
    ...
    const sale = await request(app.getHttpServer())
      ...
      .send({
        ...
        holderKey,
        ticketTypeId: ticketType.id,
        cashDrawerId: drawer.body.id,
        cashCents: 100,
        cashReceivedCents: 100,
        cardCents: quote.body.totalCents - 100,
    ...
    const refundPayload = { requestId: refundRequestId, reason: "Pending card-present refund", cashDrawerId: drawer.body.id };
    ...
    await request(app.getHttpServer())
      ...
      .send(refundPayload)
      .expect(201);
    const otherDrawer = await request(app.getHttpServer()).post("/api/v1/box-office/cash-drawers")
      .set("Authorization", `Bearer ${ownerLogin.body.accessToken}`)
      .send({ requestId: crypto.randomUUID(), registerId: `PENDING-OTHER-${crypto.randomUUID()}`, openingBalanceCents: 20_000 })
      .expect(201);
    await request(app.getHttpServer())
      .post(`/api/v1/box-office/orders/${sale.body.id}/refund`)
      .set("Authorization", `Bearer ${ownerLogin.body.accessToken}`)
      .send({ ...refundPayload, cashDrawerId: otherDrawer.body.id })
      .expect(409);
    ...
    expect(await prisma.cashTransaction.count({ where: { ticketOrderId: sale.body.id, type: "REFUND" } })).toBe(1);
  ...
  it("redeems an active gift card atomically and validates concurrent refund replays", async () => {
    ...
    const ticketType = await prisma.ticketType.findFirstOrThrow({ where: { locationId: owner.locationId, active: true } });
    const issued = await request(app.getHttpServer()).post("/api/v1/management/gift-cards")
      .set("Authorization", `Bearer ${accessToken}`).send({ amountCents: 100_000 }).expect(201);
