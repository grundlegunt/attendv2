...
test("manager signs in and reaches reporting and configuration", async ({ page }) => {
  ...
  await expect(adminNavigation.getByRole("link", { name: "Overview", exact: true })).toBeVisible();
  await expect(page.getByRole("heading", { name: "Schedule" })).toBeVisible();
  const reportsCategory = adminNavigation.getByRole("link", { name: "Reports & Finance", exact: true });
  const reportsFlyout = adminNavigation.getByLabel("Reports & Finance pages");
  await expect(reportsFlyout.getByRole("link", { name: "Revenue Reports" })).toBeHidden();
  await reportsCategory.hover();
  await expect(reportsFlyout.getByRole("link", { name: "Revenue Reports" })).toBeVisible();
  await reportsFlyout.getByRole("link", { name: "Revenue Reports" }).click();
  ...
  const reportsTabs = page.getByRole("navigation", { name: "Reports & Finance pages" });
  await expect(reportsTabs.getByRole("link", { name: "Revenue Reports" })).toHaveAttribute("aria-current", "page");
  await adminNavigation.getByRole("link", { name: "Users & Permissions", exact: true }).click();
