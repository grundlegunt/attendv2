...
- **This list intentionally stops at Milestone 11.** Real gaps and expansion opportunities beyond the MVP (external distributor/Comscore integration, nonprofit membership/donation handling, wallet passes, and more) are tracked in `POST_MVP_BACKLOG.md` rather than appended here, so the roadmap to a real, sellable product stays distinct from open-ended future scope.
