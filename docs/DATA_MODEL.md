...
### Movies & Showtimes
- **Movie**: id, organizationId, title, rating, runtimeMinutes, synopsis, posterUrl, director, starring (display string), trailerUrl, releaseYear, active. “Now Playing” and “Coming Soon” are derived from the movie's on-sale showtime dates rather than stored as competing status fields. `runtimeMinutes` is treated as the movie's full length through the end of the credits — this is the operator's convention and the basis for scheduling (below).
- **Showtime**: id, movieId, auditoriumId, priceTierId, optional filmSeriesId, startsAt (advertised doors time), featureStartsAt, endsAt, roomReadyAt, onSale, presentation, and optional **format** (for example DCP or 35mm). Format belongs to the screening because the same movie may be presented differently on different dates. Computed timing drives turnover and auto-settlement; customer-facing listings show the advertised start time.
- **MoviePairing**: movieId, menuItemId, sortOrder — display-only recommendations connecting a movie to active food and drink items. Pairings do not alter restaurant ordering or prices.
