...
- `EXPIRED` is reachable only from `AWAITING_PAYMENT` when the underlying `SeatHold`(s) expire before the processor confirms payment — handled by the reconciliation rule in PAYMENT_FLOW.md §5 (payment succeeding after expiry).
...
- `SETTLEMENT_PENDING` is the guard against double-charging: a tab can only have one `Payment` in a non-terminal state at a time; the automatic settlement job (PAYMENT_FLOW.md §6) checks this before attempting a charge and is itself idempotent per tab per billing cycle via `Payment.idempotencyKey = tab.id + settlementAttemptWindow`.
